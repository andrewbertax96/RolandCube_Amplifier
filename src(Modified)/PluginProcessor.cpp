...
void RolandCubeAudioProcessor::processBlock (AudioBuffer<float>& buffer, MidiBuffer& midiMessages)
{
   ...
   /* auto driveValue = static_cast<float> (driveParam->load());
    ...
    // Apply EQ
        eq4band.process(bufferReader_R, bufferWriter_R, midiMessages, numSamples, totalNumInputChannels, sampleRate);

        eq4band2.process(bufferReader_L, bufferWriter_L, midiMessages, numSamples, totalNumInputChannels, sampleRate);
        //for (int ch = 0; ch < buffer.getNumChannels(); ++ch)
        //{
        //    // Apply EQ
        //    if (ch == 0) {
        ...
        // Master Volume 
        // Apply ramped changes for gain smoothing
        if (masterParam.get() == previousMasterValue)
        {
            buffer.applyGain(masterParam.get());
        }
        else {
            buffer.applyGainRamp(0, (int) buffer.getNumSamples(), previousMasterValue, masterParam.get());
            previousMasterValue = masterParam.get();
        }
